...
enum class Room(
    ...
    ENGINEERING({ l -> Bonus(globalPlanetBonus = PlanetBonus(mineRate = 1.25 + (l-1) * 0.15)) }),
    FORGE({ l -> Bonus(production = ProductionBonus(smeltSpeed = 1.1 + (l-1) * 0.1)) }),
